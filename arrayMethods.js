...
// CODE HERE
const totalWithTax = orders.map(order => order.price + (order.price * order.tax))
...
let bobsTotal = purchases.filter((order) => order.owner === "Bob").reduce((total, current) => total + current.price, 0)

// let allPrice = purchases.reduce((total, current) => total += current.price)
